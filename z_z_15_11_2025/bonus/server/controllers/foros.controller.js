...
import { Opinion } from "../models/opiniones.model.js";
...
const forosController = {
    ...
    updateOne: async (req, res)=> {
        ...
        const id = req.params.id;
        try{
            const savedOpinion = await Opinion.create({opinion})
            const UpdatedForo = await Foros.findByIdAndUpdate(id, {$push: {opinions: savedOpinion}}, {new:true});
            return res.status(200).json(UpdatedForo);
        }catch(e){
            console.log(e)
            return res.status(400).json(e);
        }
